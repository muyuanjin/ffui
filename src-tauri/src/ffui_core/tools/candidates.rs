use super::resolve::{custom_path_for, downloaded_tool_path, resolve_in_path, tool_binary_name, looks_like_bare_program_name};
...
pub fn tool_candidates(
    ...
    let path_candidate = resolve_in_path(&bin).map(|p| p.to_string_lossy().into_owned()).unwrap_or(bin.clone());
    ...
    for (path, source) in candidates {
        if source == "path" && runtime.path_arch_incompatible { continue; }
        if verify_tool_binary(&path, kind, &source) {
            ...
            result.push(ExternalToolCandidate { kind, path, source, version, is_current });
        }
    }
    result
}
