...
/// 当 Smart Scan 配置中的 video_preset_id 在预设列表中不存在时，后台会将匹配到的
/// 视频视为“已统计候选并立即处理完成”，但不会入队任何子任务。此时批次应当被视为
/// 已完成，delete_smart_scan_batch 也必须返回 true 以便前端可以删除该复合任务。
#[test]
fn delete_smart_scan_batch_succeeds_for_candidates_without_children() {
    let dir = env::temp_dir().join("ffui_smart_scan_no_preset_delete");
    ...
    // 等待 Smart Scan 批次统计结果稳定：应有 >=1 个候选且已全部处理完。
    let summary = {
        let mut attempts = 0;
        loop {
            ...
            if attempts > 100 {
                panic!(
                    "Smart Scan batch did not reach expected summary (candidates without children) within timeout"
                );
            }
    ...
    assert!(
        summary.total_candidates >= 1,
        "no-preset Smart Scan batch should report at least one candidate"
    );
    assert!(
        summary.total_processed >= summary.total_candidates,
        "no-preset Smart Scan batch should count all candidates as processed immediately"
    );
    ...
    assert!(
        ...
        "no-preset Smart Scan batch must not enqueue any child jobs"
    );

    // 修复之后，delete_smart_scan_batch 必须视这种“无子任务但所有候选已处理完成”的
    // 批次为可删除，并清理掉批次元数据，避免前端出现无法删除的空压缩任务。
    assert!(
        engine.delete_smart_scan_batch(&batch_id),
        "delete_smart_scan_batch must succeed for batches with candidates but no child jobs (e.g. missing preset)"
    );
