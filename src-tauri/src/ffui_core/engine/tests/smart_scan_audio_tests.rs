...
#[test]
fn smart_scan_enqueues_audio_candidates_when_enabled() {
    let dir = env::temp_dir().join("ffui_smart_scan_audio_candidates");
    ...
    let mut config = SmartScanConfig {
    ...
    let descriptor = engine
        ...
        .expect("run_auto_compress should succeed for audio-only Smart Scan test");
    ...
    let summary = {
        ...
        loop {
            ...
            attempts += 1;
            if attempts > 100 {
                panic!("Smart Scan audio batch did not reach expected summary within timeout");
            }
    ...
    assert_eq!(
        summary.total_files_scanned, 1,
        "audio-only Smart Scan tree contains exactly one file"
    );
    assert_eq!(
        summary.total_candidates, 1,
        "audio-only Smart Scan must treat the mp3 file as a single candidate"
    );
    assert_eq!(
        summary.total_processed, 1,
        "audio-only Smart Scan candidate must be immediately marked as processed"
    );
    ...
    assert_eq!(
        audio_jobs.len(),
        1,
        "Smart Scan must enqueue exactly one audio job for the mp3 candidate"
    );
