...
fn run_auto_compress_progress_listener_can_call_queue_state_without_deadlock() {
    ...
    let summary = {
        ...
        loop {
            ...
            attempts += 1;
            if attempts > 100 {
                panic!("Batch Compress batch did not finish within timeout in lock-free test");
            }
            std::thread::sleep(std::time::Duration::from_millis(50));
        }
    };
...
fn batch_compress_pushes_full_batch_snapshot_after_detection() {
    ...
    loop {
        ...
        attempts += 1;
        if attempts > 100 {
            panic!("Batch Compress queue snapshot did not include all children within timeout");
        }
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
