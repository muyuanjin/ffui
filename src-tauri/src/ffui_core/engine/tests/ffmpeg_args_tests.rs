...
#[test]
fn build_ffmpeg_args_normalizes_wmv_container_to_asf_muxer() {
    let mut preset = make_test_preset();
    preset.container = Some(ContainerConfig {
        format: Some("wmv".to_string()),
        movflags: None,
    });
    ...
    assert!(
        joined.contains("-f asf"),
        "preset with container.format=wmv must emit -f asf, got: {joined}"
    );
}

#[test]
fn build_ffmpeg_args_normalizes_ts_and_m2ts_to_mpegts_muxer() {
    for fmt in ["ts", "m2ts"] {
        let mut preset = make_test_preset();
        preset.container = Some(ContainerConfig {
        ...
        assert!(
            ...
            "preset with container.format={fmt} must emit -f mpegts, got: {joined}"
        );
    }
}
...
#[test]
fn build_ffmpeg_args_forced_webm_falls_back_to_matroska_when_incompatible() {
    let mut preset = make_test_preset();
    ...
    assert!(
        joined.contains(" -f matroska "),
        "incompatible forced webm must fall back to -f matroska, got: {joined}"
    );
    assert!(
