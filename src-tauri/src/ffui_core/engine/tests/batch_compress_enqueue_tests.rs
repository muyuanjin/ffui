...
fn batch_compress_video_enqueue_keeps_batch_children_consecutive_even_when_manual_interleaves() {
    use crate::ffui_core::engine::batch_compress::video::enqueue_batch_compress_video_job;
    ...
    let job1 = enqueue_batch_compress_video_job(
        &engine.inner,
        &input1,
        &config,
        &settings,
    ...
    assert!(
        job1.ffmpeg_command
            ...
            .unwrap_or_default()
            .contains("ffmpeg"),
        "Batch Compress queued job should carry a planned ffmpeg command before processing"
    );
    ...
    let job2 = enqueue_batch_compress_video_job(
        &engine.inner,
        &input2,
        &config,
        &settings,
