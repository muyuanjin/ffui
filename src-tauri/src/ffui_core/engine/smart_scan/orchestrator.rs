...
fn run_auto_compress_background(
    ...
    for path in all_files {
        ...
        if is_image_file(&path) {
            ...
            update_smart_scan_batch_with_inner(&inner, &batch_id, true, |batch| {
                ...
                if matches!(batch.status, SmartScanBatchStatus::Scanning) {
    ...
    if !pending_media_tasks.is_empty() {
        ...
        thread::Builder::new()
            .name(format!("smart-scan-media-worker-{batch_id}"))
            .spawn(move || {
                for (job_id, path, kind) in pending_media_tasks {
                    ...
                    match result {
                        ...
                        Err(err) => {
                            ...
                            if let Some(job) = state.jobs.get_mut(&job_id) {
                                ...
                                let reason =
                                    format!("Smart Scan media compression failed: {err:#}");
                    ...
                    mark_smart_scan_child_processed(&inner_clone, &job_id);
                }
            })
            .expect("failed to spawn smart scan media worker thread");
    }
}
