...
fn pending_queue_lite_delta_merges_sparse_patches_in_revision_order() {
    ...
    assert_eq!(
        ...
        Some("C:/previews/job-1.jpg")
    );
    assert_eq!(
        patch.preview.as_ref().and_then(|p| p.preview_revision),
        Some(5)
    );
}
...
fn merge_queue_state_lite_delta_patch_applies_all_fields() {
    ...
    assert_eq!(
        into.telemetry.as_ref().and_then(|t| t.last_progress_frame),
        Some(9)
    );
    assert_eq!(
        into.preview
