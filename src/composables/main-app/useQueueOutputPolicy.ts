...
export function useQueueOutputPolicy(appSettings: Ref<AppSettings | null>) {
  const queueOutputPolicy = computed<OutputPolicy>(
    () => appSettings.value?.queueOutputPolicy ?? DEFAULT_OUTPUT_POLICY,
  );
  ...
  const setQueueOutputPolicy = (policy: OutputPolicy) => {
    ...
    const nextSettings: AppSettings = {
      ...(current ??
        ({ tools: {}, smartScanDefaults: {}, previewCapturePercent: 50 } as AppSettings)),
