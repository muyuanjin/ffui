...
export const resetExitPrompt = async (): Promise<void> => {
  if (!hasTauri()) return;
  await invokeWithAliases<void>("reset_exit_prompt");
};

export const exitAppNow = async (): Promise<void> => {
  if (!hasTauri()) return;
  await invokeWithAliases<void>("exit_app_now");
};
...
export const exitAppWithAutoWait = async (): Promise<ExitAutoWaitOutcome> => {
  ...
  return invokeWithAliases<ExitAutoWaitOutcome>("exit_app_with_auto_wait");
};
