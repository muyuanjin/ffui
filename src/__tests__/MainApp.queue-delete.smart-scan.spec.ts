...
describe("MainApp queue delete behaviour (Smart Scan batches)", () => {
  it("批次所有子任务均为终态时，Smart Scan 批次可以整体删除且不报错", async () => {
    ...
    const batchDeleteCalls = invokeMock.mock.calls.filter(([cmd]) => cmd === "delete_smart_scan_batch");
  ...
  it("当批次仍有非终态子任务时，Smart Scan 子任务删除被拒绝并提示 deleteActiveNotAllowed", async () => {
    ...
    expect(
      invokeMock.mock.calls.some(([cmd]) => cmd === "delete_transcode_job" || cmd === "delete_smart_scan_batch"),
    ).toBe(false);
