...
describe("backend invoke commands are registered in Rust", () => {
  it("ensures every invoke() command name exists in src-tauri/src/lib.rs generate_handler list", () => {
    ...
    const handlerMatch = rustSource.match(
    ...
    const rustCommandMatches = Array.from(
      handlerList.matchAll(/::([a-z0-9_]+)\s*(?:,|\s*$)/g),
    );
    ...
    expect(
      missing,
      `missing Rust command registrations for: ${missing.join(", ")}`,
    ).toEqual([]);
  });
});
