...
const basePreset: FFmpegPreset = {
  id: "preset-1",
  name: "Test Preset",
  description: "Preset used in QueuePanel Smart Scan children tests",
  video: {
    encoder: "libx264",
    rateControl: "crf",
    qualityValue: 23,
...
describe("QueuePanel Smart Scan batch children interactions", () => {
  it("允许在队列模式下对复合任务展开的子任务进行单独选中和右键", async () => {
    ...
    const childrenContainer = wrapper.get("[data-testid='smart-scan-batch-children']");
  ...
  it("允许在队列模式下整体右键复合任务卡片以对该批次子任务执行批量操作", async () => {
    ...
    const batchCard = wrapper.get("[data-testid='smart-scan-batch-card']");
