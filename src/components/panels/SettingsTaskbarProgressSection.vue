...
const titlebarProgressEnabled = computed(() => props.appSettings?.titlebarProgressEnabled ?? true);
  ...
  <Card class="border-border/50 bg-card/95 shadow-sm flex flex-col" data-testid="settings-card-progress-display">
    ...
    <CardContent v-if="appSettings" class="p-2 flex flex-col flex-1">
      <div class="flex flex-col flex-1 justify-between gap-3">
        <div
          class="rounded border border-border/40 bg-muted/10 px-2 py-2"
          data-testid="settings-titlebar-progress-section"
        >
          <div class="flex items-start justify-between gap-3">
            <div class="space-y-0.5">
              <p class="text-[11px] font-medium text-foreground">
                {{ t("app.settings.titlebarProgressEnabledLabel") }}
              </p>
              <p class="text-[10px] text-muted-foreground leading-snug">
                {{ t("app.settings.titlebarProgressEnabledHelp") }}
              </p>
            </div>
            <Switch
              data-testid="settings-titlebar-progress-enabled"
              :model-value="titlebarProgressEnabled"
              @update:model-value="(v) => updateSetting('titlebarProgressEnabled', !!v)"
            />
          </div>
        </div>
        <div>
