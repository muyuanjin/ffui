...
type SavingConditionType = SmartScanConfig["savingConditionType"];
  ...
  <div class="space-y-3 p-3 rounded-lg border border-border/60 bg-muted/30">
    ...
    <div v-if="savingConditionType === 'ratio'" class="space-y-2">
      ...
      <p class="text-[10px] text-muted-foreground">
        {{ t("smartScan.minSavingRatioHelp", { ratio: (minSavingRatio * 100).toFixed(0) }) }}
      </p>
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      <p class="text-[10px] text-muted-foreground">
        {{ t("smartScan.minSavingAbsoluteHelp", { size: minSavingAbsoluteMb }) }}
      </p>
    </div>
  </div>
</template>
