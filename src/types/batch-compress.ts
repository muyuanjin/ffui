...
export interface BatchCompressConfig {
  /** 扫描根路径 */
  rootPath?: string;
  /** 是否替换原文件（默认 true） */
  ...
  /** Output policy for this Batch Compress run (container/dir/name/timestamps). */
...
/**
 * Frontend-side aggregation of a Batch Compress batch. A single batch can contain
 ...
 */
export interface CompositeBatchCompressTask {
  batchId: string;
  rootPath: string;
  jobs: TranscodeJob[];
  totalFilesScanned: number;
