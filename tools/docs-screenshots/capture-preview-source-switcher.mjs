...
const parseArgs = () => {
  ...
  const parsed = {
    outDir: path.join(repoRoot, "tools/docs-screenshots/artifacts/preview-source-switcher"),
    locale: "zh-CN",
    port: 5173,
    width: 980,
    height: 720,
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (a === "--help" || a === "-h") {
      ...
      console.log(
        [
          "Usage:",
          "  node tools/docs-screenshots/capture-preview-source-switcher.mjs [options]",
          "",
          "Options:",
...
const withDevServer = async (fn, options = {}) => {
  ...
  const env = {
    ...
    BROWSER: "none",
    LANG: "en_US.UTF-8",
    // Keep the UI fully hydrated (mirrors other capture scripts).
  ...
  await withDevServer(
    async ({ baseUrl }) => {
      ...
      try {
        ...
        await page.locator("[data-testid='queue-item-card']").first().waitFor({ state: "visible", timeout: 60_000 });

        await page.locator("[data-testid='queue-item-thumbnail']").first().click({ force: true });
