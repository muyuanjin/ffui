...
- `FFUI_METRICS_INTERVAL_MS`  
  - 单位：毫秒  
  - 含义：系统性能采样的基础间隔（后端当前实现始终按该间隔采样并推送事件）。  
  - 默认：`1000`（1 秒）。

- `FFUI_METRICS_IDLE_INTERVAL_MS`  
  - 单位：毫秒  
  - 含义：预留给未来“按订阅数动态降频”的空闲休眠配置；当前实现中不会实际使用该值。  
  - 默认：`5000`（5 秒，保留值）。
...
- 视图组件：`MonitorPanel.vue`（`src/components/panels/MonitorPanel.vue`）
  ...
  - NVIDIA GPU 使用率 / 显存使用率（基于 NVML，通过 `system-metrics://update` 事件流式推送，而非轮询 `get_gpu_usage` 命令）。
